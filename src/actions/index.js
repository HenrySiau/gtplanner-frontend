...
import { DRAWER_EXTEND, DRAWER_FOLD, UPDATE_USER_INFO, ADD_MEMBER, CHANGE_CHAT_ROOM_TABS_VALUE, UPDATE_FILTERED_MARKER_LIST } from './actionTypes';
...
import { UPDATE_FILTERED_IDEAS, ADD_FILTERED_IDEA , UPDATE_FOCUSED_IDEA} from './actionTypes';
import { makeMarker } from '../components/utility/mapFunctions';
...
export function loginWithPassword(email, password, invitationCode) {
    return function (dispatch) {
...
export const addFilteredIdea = (idea) => {
    let markerInfo = {
        id: idea.id,
    ...
    return ({
        type: ADD_FILTERED_IDEA,
        idea: idea,
    });
}
export const updateFocusedIdea = (ideaId) => {
    return ({
        type: UPDATE_FOCUSED_IDEA,
        ideaId: ideaId,
    });
}